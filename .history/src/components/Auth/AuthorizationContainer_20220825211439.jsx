...
import { setAuthUserData, setMyUserProfile } from '../../redux/auth-reducer';
        ...
        axios.get(`http://localhost:3001/users/3` + '?_embed=auth')
            .then(response => {
                if (response.data.resultCode === 0) {
                    let { id, email, login } = response.data.auth[0].data
                    console.log(response.data.auth[0].data);
...
export default connect(mapStateToProps,
    { setAuthUserData, setMyUserProfile })(AuthorizationContainer)