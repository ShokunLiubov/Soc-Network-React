...
export const profileAPI = {
    ...
    getStatusProfile(profileId) {
        return instance.get('status' + profileId).then(response => response.data.status[0].status)
    },
    updateStatusProfile(status, profileId) {
        return instance.put('status' + profileId, {status}).then(response => response.data.status[0].status)
    }
}
