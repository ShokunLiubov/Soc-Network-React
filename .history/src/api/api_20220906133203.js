...
export const profileAPI = {
    ...
    getStatusProfile(profileId) {
        return instance.get('users/' + profileId + '?_embed=status').then(response => response.data.profile[0].status)
    },
    updateStatusProfile(status, profileId) {
        return instance.put('users/' + profileId + '?_embed=status', {status}).then(response => response.data.profile[0].status)
    }
}
